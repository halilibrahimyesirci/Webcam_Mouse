...
class Calibrator:
    def __init__(self):
        ...
        self.feature_version = 2  # Özellik vektörü versiyonu
    ...
    def _create_feature_vector(self, gaze_point, version=None):
        ...
        if version == 1:  # Eski versiyon - temel özellikler
            return np.array([
                1.0,  # Bias
                gaze_point[0], gaze_point[1],  # Linear
                gaze_point[0] * gaze_point[1],  # Cross term
                gaze_point[0]**2              # Single quadratic term
            ])
        else:  # Yeni versiyon - gelişmiş özellikler
            return np.array([
                1.0,  # Bias
                gaze_point[0], gaze_point[1],  # Linear
    ...
    def get_screen_position(self, gaze_point):
        ...
        version = self._detect_matrix_version(matrix)
        
        # Uygun versiyonda özellik vektörü oluştur
        gaze_features = self._create_feature_vector(gaze_point, version)
    ...
    def calculate_transformation(self):
        ...
        for depth in self.calibration_data:
            ...
            try:
                U, S, Vh = np.linalg.svd(gaze_features, full_matrices=False)
                # Küçük tekil değerleri filtrele (regularizasyon)
                S_inv = np.array([1/s if s > 1e-10 else 0 for s in S])
                self.transformation_matrices[depth] = (Vh.T * S_inv) @ U.T @ points
